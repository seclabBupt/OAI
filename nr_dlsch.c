...
uint8_t nr_generate_pdsch(PHY_VARS_gNB *gNB,
  ...
  for (int dlsch_id=0;dlsch_id<gNB->number_of_nr_dlsch_max;dlsch_id++) {
	...
	int16_t * input_otfs = &(txdataF_precoding[ap][((rel15->StartSymbolIndex * frame_parms->ofdm_symbol_size)<<1) + (2*txdataF_offset)]);
	PHY_otfs_mod(input_otfs,rel15->NrOfSymbols,frame_parms->ofdm_symbol_size,input_otfs);
	printf("OTFS modulation is on !\n");
      }
    }// layer loop
