  ...
  */
  ...
  if (dci_cnt > 0) {
    ...
    if (dlsch) {
      ...
      if(IS_SOFTMODEM_OTFS){
        for(unsigned char aa = 0; aa<ue->frame_parms.nb_antennas_rx; aa++){
	  short *otfs_input = (int16_t *)&(ue->common_vars.common_vars_rx_data_per_thread->rxdataF[aa][ue->frame_parms.ofdm_symbol_size * start_symb_sch]);
          PHY_otfs_demod(otfs_input,nb_symb_sch,ue->frame_parms.ofdm_symbol_size,otfs_input);
        }
      }
